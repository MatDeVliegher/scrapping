...
def get_townhall_urls(page)
  page = Nokogiri::HTML(open("http://annuaire-des-mairies.com/val-d-oise.html"))
  array_urls = page.xpath("//a[@class='lientxt']/@href")  #on récupère les liens de chaque ville
  array_urls = array_urls.map { |url| url.to_s } # qu'on transforme en string
  array_urls = array_urls.map { |url| url = url[1..-1] }
  array_urls = array_urls.map { |url| url.prepend("http://annuaire-des-mairies.com") } # permet de rajouter le bour d'URL différent pour chaque ville à l'annuaire de base
  return array_urls
end
